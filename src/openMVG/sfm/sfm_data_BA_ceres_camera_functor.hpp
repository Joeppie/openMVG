    ...
    */
            ...
            )
    {
      ...
      if (weight == 0.0)
      {
        return
                (new ceres::AutoDiffCostFunction
                        <ResidualErrorFunctor_Intrinsic_Spherical_Equirectangular, 2, 6, 3>(
                        new ResidualErrorFunctor_Intrinsic_Spherical_Equirectangular(
                                observation.data(), cameraInterface->w(), cameraInterface->h())));
      }
      else
      {
        return
                (new ceres::AutoDiffCostFunction
                        <WeightedCostFunction<ResidualErrorFunctor_Intrinsic_Spherical_Equirectangular>, 2, 6, 3>
                        (new WeightedCostFunction<ResidualErrorFunctor_Intrinsic_Spherical_Equirectangular>
                                 (new ResidualErrorFunctor_Intrinsic_Spherical_Equirectangular(
                                         observation.data(), cameraInterface->w(), cameraInterface->h()),
                                  weight)));
      }
